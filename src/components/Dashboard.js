...
function Dashboard() {
    ...
    const saveUser = async () => {
        ...
        if(user._id){
            ...
            (response === true) && toast.current.show({ severity: 'success', summary: 'Successful', detail: 'User Updated', life: 3000 });
        }
        else{
            const response = await user_context.addUser(user);
            (response === true) && toast.current.show({ severity: 'success', summary: 'Successful', detail: 'User Created', life: 3000 });
        }
        setUserDialog(false);
    ...
    const leftToolbarTemplate = () => {
        return (
            <div className="flex flex-wrap gap-2">
                {role === 1 &&<Button label="New" icon="pi pi-plus" severity="success" onClick={openNew} />}
            </div>
        );
    };
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <React.Fragment>
                ...
                {role === 1 && <Button icon="pi pi-trash" rounded outlined severity="danger" onClick={() => confirmDeleteUser(rowData)} />}
            </React.Fragment>
        );
    };
    ...
    return (
        ...
        {role !== 3 && <div>
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={user_context.usersList} selection={selectedUsers} onSelectionChange={(e) => setSelectedUsers(e.value)}
                    ...
                    <Column field="dob" header="Date Of Birth" sortable style={{ minWidth: '16rem' }}></Column>
                    <Column field="city" header="City" sortable style={{ minWidth: '16rem' }}></Column>
                    <Column field="state" header="State" sortable style={{ minWidth: '16rem' }}></Column>
                    {(role === 1 || role === 2) && <Column body={actionBodyTemplate} exportable={false} style={{ minWidth: '12rem' }}></Column> }
                </DataTable>
            </div>

            <Dialog visible={userDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="User Details" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                ...
                <div className="field">
                    ...
                    {submitted && !user.first_name && <small className="p-error">First Name is required.</small>}
                </div>
                <div className="field">
                    ...
                    {submitted && !user.last_name && <small className="p-error">Last Name is required.</small>}
                </div>
                <div className="field">
                ...
                <div className="field">
                    ...
                    <InputText id="city" value={user.city} onChange={(e) => onInputChange(e, 'city')} required autoFocus className={classNames({ 'p-invalid': submitted && !user.city })} />
                ...
                <div className="field">
                    ...
                    <InputText id="state" value={user.state} onChange={(e) => onInputChange(e, 'state')} required autoFocus className={classNames({ 'p-invalid': submitted && !user.state })} />
                    {submitted && !user.state && <small className="p-error">State is required.</small>}
                </div>
                <div className="field">
                    <label htmlFor="password" className="font-bold">
                        Password
                    </label>
                    <InputText id="password" onChange={(e) => onInputChange(e, 'password')} autoFocus/>
                </div>
                <div className="field">
                    <label htmlFor="description" className="font-bold">
                        Date Of Birth
                    </label>
                    <Calendar value={new Date(user.dob)} onChange={(e) => onInputChange(e, 'dob')} />
                </div>
                ...
                <div className="field">
                    ...
                    <div className="formgrid grid">
                        <div className="field-radiobutton col-6">
                            <RadioButton inputId="role1" name="manager" value="1" onChange={onRoleChange} checked={user.role === '1'} />
                            <label htmlFor="role1">Manager</label>
                        </div>
                        <div className="field-radiobutton col-6">
                            <RadioButton inputId="role2" name="admin" value="2" onChange={onRoleChange} checked={user.role === '2'} />
                            <label htmlFor="role2">Admin</label>
                        </div>
                        <div className="field-radiobutton col-6">
                            <RadioButton inputId="role3" name="user" value="3" onChange={onRoleChange} checked={user.role === '3'} />
